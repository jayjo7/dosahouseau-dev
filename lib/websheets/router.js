						...
						waitOn:  function()
								{
							        ...
							        Meteor.subscribe('workhours', 					websheets.public.generic.ORG_NAME);	       
							        console.log(appUUID + ':done subscribing to workHours...');		
				 						...
				 						{ 
									        ...
									        Session.setPersistent(websheets.public.generic.ORG_NAME_SESSION_KEY, 	websheets.public.generic.ORG_NAME);
				 						    ...
				 						    Meteor.subscribe('ordersMeta', this.params.UniqueId, 					websheets.public.generic.ORG_NAME);
				 						   	console.log(appUUID + ':done subscribing to ordersMeta...');
				 						   	Meteor.subscribe('workhours', 					websheets.public.generic.ORG_NAME);	       
							        		console.log(appUUID + ':done subscribing to workHours...');						 						   	var order = Orders.findOne({UniqueId:this.params.UniqueId, orgname:websheets.public.generic.ORG_NAME});
				 						   	var omEnabled = checkEnabled(Meteor.settings.public[websheets.public.generic.ORG_NAME].osm.toUpperCase());
				 							return {UniqueId: this.params.UniqueId, Order:order, omEnabled:omEnabled};
